...
FEATURED_CSV = os.path.join(PROJECT_ROOT, 'backend', 'data', 'processed', 'paddy_common_2021_2026_featured.csv')
...
def build_weighted_avg_dataset():
    print("="*85)
    print("BUILDING DATASET FOR WEIGHTED AVERAGE MODAL PRICE PREDICTION (2021 - 2026)")
    print("="*85)
    ...
    for (mkt, dist) in top_markets:
        ...
        if not m_df.empty:
            res = pd.merge(res, m_df[['date', 'modal_price', 'min_price', 'max_price']], on='date', how='left')
        else:
        ...
        # CALCULATE WEIGHTED AVERAGE MODAL PRICE: (2 * Modal + Min + Max) / 4
        res['weighted_avg_modal_price'] = (2.0 * res['modal_price'] + res['min_price'] + res['max_price']) / 4.0
        ...
        # Price Lags & Targets on WEIGHTED AVERAGE MODAL PRICE
        p = res['weighted_avg_modal_price']
        res['target_modal_price'] = p.shift(-1) # Target next day weighted avg modal price
    ...
    print(f"  Saved Weighted Average Modal Price featured dataset: {FEATURED_CSV}")
