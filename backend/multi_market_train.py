...
def train_and_evaluate_multi_market():
    ...
    print("[Multi-Market Trainer] Loading multi-market panel dataset...")
    df = pd.read_csv(csv_path)
    df['date'] = pd.to_datetime(df['date'])
    df = df.sort_values('date').reset_index(drop=True)
    
    # Feature columns including stationary indicators, weather, & market encodings
    ...
    print(f"\n[Multi-Market Trainer] Chronological Panel Split Across AP Markets:")
    print(f"  Total Panel Rows: {n}")
    print(f"  Train Set : {len(train_df)} rows ({train_df['date'].min().strftime('%Y-%m-%d')} to {train_df['date'].max().strftime('%Y-%m-%d')})")
    print(f"  Val Set   : {len(val_df)} rows ({val_df['date'].min().strftime('%Y-%m-%d')} to {val_df['date'].max().strftime('%Y-%m-%d')})")
    ...
    def evaluate_predictions(y_true, y_pred, y_today):
        ...
        # Output prediction distribution std (checking for under-learning / zero variance)
        pred_std = np.std(pred_diff)
        return {'MAE': round(mae, 2), 'RMSE': round(rmse, 2), 'MAPE': round(mape, 2), 'DirAcc': round(dir_acc, 2), 'PredDiffStd': round(pred_std, 3)}
    ...
    print("\n" + "="*95)
    print(f"{'MULTI-MARKET MODEL PERFORMANCE & PREDICTION VARIANCE EVALUATION':^95}")
    print("="*95)
    ...
    for name, res in results.items():
        v = res['val']
        t = res['test']
        print(f"{name:<12} | {v['MAPE']:>8.2f}% | Rs.{v['MAE']:>6.1f} | {v['DirAcc']:>8.1f}% | Rs.{v['PredDiffStd']:>8.2f} || {t['MAPE']:>8.2f}% | Rs.{t['MAE']:>6.1f} | {t['DirAcc']:>8.1f}% | Rs.{t['PredDiffStd']:>8.2f}")
    ...
    model_path = os.path.join(models_dir, 'rice_ap_multi_market_model.joblib')
    joblib.dump(model_artifact, model_path)
    print(f"[Multi-Market Trainer] Saved best model artifact to: {model_path}")
    
    return model_artifact

if __name__ == '__main__':
    train_and_evaluate_multi_market()
